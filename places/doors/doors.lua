if not ExecutorSupport then print("[YEHAX] Loading stopped, please use the official loadstring for mspaint. (ERROR: ExecutorSupport == nil)") return end
if getgenv().mspaint_loaded then print("[YEHAX] Loading stopped. (ERROR: Already loaded)") return end
...
Visuals = Window:AddTab("透视物品"),
Floor = Window:AddTab("楼层功能"),
["UI Settings"] = Window:AddTab("UI介绍"),
}
...
function Script.Functions.UpdateRPC()
    ...
    BloxstrapRPC.SetRichPresence({
        details = "Playing DOORS [ XSKIL V3.0 ]",
...
function Script.Functions.AutoWardrobe(child, index: number | nil) -- child = entity, ty upio
    ...
    local NotifPrefix = "自动躲" .. HidingPlaceName[floor.Value];
    if Toggles.AutoWardrobeNotif.Value then Script.Functions.Log("[" .. NotifPrefix .. "] 寻找柜子...") end
    ...
    -- Hide
    if Toggles.AutoWardrobeNotif.Value then Script.Functions.Log("[" .. NotifPrefix .. "] 开始..") end
        ...
        if child.Name == "A120" then
        	...
        	repeat task.wait(.15)
                ...
                if didPlayerSeeEntity == true and isSafe == true then
                    if Toggles.AutoWardrobeNotif.Value then Script.Functions.Log("[" .. NotifPrefix .. "] 正在离开柜子,实体在远处.") end              
                	break
                else
                    if isSafe == true and not child:IsDescendantOf(workspace) then 
                        if Toggles.AutoWardrobeNotif.Value then Script.Functions.Log("[" .. NotifPrefix .. "] 离开柜子,实体已被删除.") end           
                    	break 
                    end          
                end

                if not alive then  
                    if Toggles.AutoWardrobeNotif.Value then Script.Functions.Log("[" .. NotifPrefix .. "] 停止 (你死了孩子).") end             
                	break 
                end                             
            until false == true          
        end
...
function Script.Functions.DoorESP(room)
    ...
    if door then
        ...
        local doorState = if opened then " [已开]" elseif locked then " [上锁]" else ""
        ...
        Script.Connections[room.Name .. "Opened"] = door:GetAttributeChangedSignal("Opened"):Connect(function()
            ...
            if doorEsp then doorEsp.SetText(string.format("门 %s [开启]", doorNumber)) end
        end)
    end
end 
...
function Script.Functions.ChestESP(chest)
    ...
    Script.Functions.ESP({
        Type = "Chest",
        Object = chest,
        Text = if locked then "箱子 [锁]" else "箱子",
    ...
    Library:GiveSignal(player.CharacterAdded:Connect(function(newCharacter)
        ...
        Script.Connections[player.Name .. "ChildAdded"] = newCharacter.ChildAdded:Connect(function(child)
            if child:IsA("Tool") and child.Name:match("LibraryHintPaper") then
                ...
                if Toggles.NotifyPadlock.Value and count < 5 then
                    Script.Functions.Alert(string.format("图书馆密码: %s", output))
...
function Script.Functions.DeleteSeek(collision: BasePart)
    ...
    task.spawn(function()
        ...
        if collision:IsDescendantOf(workspace) and (collision.Parent and collision.Parent.Name == "TriggerEventCollision") then
        	Script.Functions.Alert("删除seek...")

            task.delay(4, function()
                if collision:IsDescendantOf(workspace) then
                    Script.Functions.Alert("Seek删除失败!")
                end
            end)
...
local ReachGroupBox = Tabs.Main:AddLeftGroupbox("距离") do
...
local AutomationGroupBox = Tabs.Main:AddRightGroupbox("交互类") do
    ...
    AutomationGroupBox:AddToggle("AutoWardrobeNotif", {
        Text = "自动躲" .. HidingPlaceName[floor.Value] .. " 通知",
        Default = false
    })
...
local AntiEntityGroupBox = Tabs.Exploits:AddLeftGroupbox("防实体") do
    ...
    AntiEntityGroupBox:AddToggle("AntiDupe", {
        Text = "防" .. (isBackdoor and "虚空门" or "假门"),
        Default = false
    })

    AntiEntityGroupBox:AddToggle("AntiEyes", {
        Text = "防" .. (isBackdoor and "灯泡人" or "眼睛"),
        Default = false
    })
...
local TrollingGroupBox = Tabs.Exploits:AddLeftGroupbox("恶作剧") do
...
local SelfGroupBox = Tabs.Visuals:AddRightGroupbox("自己") do
...
task.spawn(function()
    if isHotel then
        local Hotel_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("移除伤害") do
        ...
        local Hotel_ModifiersGroupBox = Tabs.Floor:AddRightGroupbox("修饰符") do
    ...
    elseif isMines then
        ...
        local Mines_AutomationGroupBox = Tabs.Floor:AddRightGroupbox("Automation") do
            Mines_AutomationGroupBox:AddButton({
                Text = "一键200门闸门",
                Func = function()
                    if latestRoom.Value < 99 then Script.Functions.Alert("未到达200门..") end
        ...
        Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
            if value then
                ...
                if Library.IsMobile then
                    Script.Functions.Alert("绕过成功!(200门和快速梯子)", progressPart)
                else
                    Script.Functions.Alert("绕过成功!(200门和快速梯子)", progressPart)
                end
    ...
    elseif isBackdoor then
        local Backdoors_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("实体") do
    ...
    elseif isRooms then
        local Rooms_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("防实体") do
        ...
        local Rooms_AutomationGroupBox = Tabs.Floor:AddRightGroupbox("Automation") do
            ...
            Rooms_AutomationGroupBox:AddLabel("建议设置:\n关闭速度旁路和穿墙", true)
        ...
        Toggles.AntiA90:OnChanged(function(value)
            if Toggles.AutoRooms.Value and not value then
                Script.Functions.Alert("防A90需要开启自动Rooms才能有效", 5)
            ...
            if value then
                ...
                while Toggles.AutoRooms.Value and not Library.Unloaded do
                    if latestRoom.Value == 1000 then
                        Script.Functions.Alert("你已到达A1000")
                        break
                    end
    ...
    elseif isFools then
        local Fools_TrollingGroupBox = Tabs.Floor:AddLeftGroupbox("恶作剧") do
        ...
        local Fools_AntiEntityGroupBox = Tabs.Floor:AddRightGroupbox("防") do
            ...
            task.spawn(function()
                ...
                if child:IsDescendantOf(workspace) then
                    ...
                    sound:Destroy()
                        ...
                        RBXGeneral:SendAsync(shortName .. " 生成了!")
                    end
                end
            end)
...
CreditsGroup:AddLabel("deividcomsono - 脚本开发")
CreditsGroup:AddLabel("upio - 脚本开发")
CreditsGroup:AddLabel("mstudio45 - 假复活 & firepp")

CreditsGroup:AddDivider()
CreditsGroup:AddLabel("汉化:")
CreditsGroup:AddLabel("XSKIL最新10月6日")
CreditsGroup:AddLabel("小玄奘 - 汉化")
...
SaveManager:SetFolder("XSKIL/doors")
